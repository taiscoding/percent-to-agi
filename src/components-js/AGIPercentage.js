...
const AGIPercentage = ({ percentage }) => {
  ...
  useEffect(() => {
    ...
    // Subscribe to the motion value changes
    const unsubscribe = displayPercentage.onChange(latest => {
      setDisplayValue(latest);
    });

    // Cleanup subscription
    return () => unsubscribe();
  }, [percentage, springPercentage, displayPercentage]);
